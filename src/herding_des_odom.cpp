...
class Sheep
{
	...
	ros::Subscriber desOdomSub;          // desired pose and velocity of sheep
	...
	Eigen::Vector2f xd;                  // desired position for the sheep
	Eigen::Vector2f xdDot;               // desired velocity of the sheep
	...
	int N;
	float lambda;
	std::vector<Eigen::Vector2f> xData, xdData, xdDotData;  // x data for the save
...
public:
	      ...
	      int NInit, float lambdaInit, bool saveDataInit, std::string runNumberInit, double height) : sheepName(sheepNameInit)
	{
		...
		desOdomSub = nh.subscribe("desOdom",1,&Sheep::desOdomCB,this);   // subscribe to desired pose and velocity 
		...
		xd = Eigen::Vector2f::Zero();
		xdDot = Eigen::Vector2f::Zero();
		xg(0) = xgInit.at(0);
		xg(1) = xgInit.at(1);
		...
		xdData.push_back(Eigen::Vector2f::Zero());
		xdDotData.push_back(Eigen::Vector2f::Zero());
	...
	Sheep(const Sheep& sheepNew) : sheepName(sheepNew.sheepName)
	{
		...
		xdData = sheepNew.xdData;
		xdDotData = sheepNew.xdDotData;
	...
	//desired odom callback
	void desOdomCB(const nav_msgs::Odometry::ConstPtr& msg)
	{
		Eigen::Vector2f xdMeas,xdDotMeas;
		xdMeas << msg->pose.pose.position.x, msg->pose.pose.position.y;
		xdDotMeas << msg->twist.twist.linear.x, msg->twist.twist.linear.y;
		xd = xdMeas;
		xdDot = xDotMeas;
		if (firstDesired)
		{
	...
	void xCB(const geometry_msgs::PoseStamped::ConstPtr& msg)
	{
		...
		Eigen::Vector2f xdNow = xd;
		Eigen::Vector2f xdDotNow = xdDot;
		Eigen::Vector2f xBar = x - xdNow;
		Eigen::Vector2f yd = K1*xBar + xdNow;
		...
		if (!killSheep)
		{
			...
			xData.push_back(x);
			xdData.push_back(xdNow);
			xdDotData.push_back(xdDotNow);
	...
	void saveRunData()
	{
		...
		if (sheepFile.is_open())
		{
			...
			sheepFile << "xd0," << "xd1," << "xdDot0," << "xdDot1,";
			sheepFile << "\n";
			...
			for (int i = 0; i < xData.size(); i++)
			{
				...
				Eigen::Vector2f xdi = xdData.at(i);
				Eigen::Vector2f xdDoti = xdDotData.at(i);
				float x0i = xi(0);
				float x1i = xi(1);
				float xd0i = xdi(0);
				float xd1i = xdi(1);
				float xdDot0i = xdDoti(0);
				float xdDot1i = xdDoti(1);
				...
				sheepFile << xd0i << "," << xd1i << "," << xdDot0i << "," << xdDot1i << ",";
				sheepFile << "\n";
			}
			sheepFile.close();
		}
...
class Bear
{
	...
	float chasingRatio;           // ratio for how much the bear should drive down the sheep before switching
...
public:
	...
	Bear(std::string runNumberInit, std::string bearNameInit, std::vector<std::string> sheepNameInit, 
	     ...
	     float chasingRatioInit, float heightInit, 
	     ...
	     float sm, float s, int L1Init, float lambdaInit) : bearName(bearNameInit)
	{
		runNumber = runNumberInit;
	...
	void yCB(const geometry_msgs::PoseStamped::ConstPtr& msg)
	{
		...
		if (chasing < 0)
		{
			...
			std::vector<float> xNorms; // L2 norm of each sheep position in xy
			for (int i = 0; i < sheep.size(); i++)
			{
				...
				if (pushToGoal)
				{
					xiNorm = (xBari.segment(0,2)).norm();
				}
				else
				{
					xiNorm = (xi.segment(0,2)).norm();
				}
				xNorms.push_back(xiNorm);
				//std::cout << "xiNorm for sheep " << i << " is " << xiNorm << std::endl;
			}
			chasing = std::distance(xNorms.begin(),std::max_element(xNorms.begin(),xNorms.end())); // chase index of maximum norm
			xNormChasingStart = xNorms.at(chasing);
			
			if ((xNormChasingStart < originRadius) && !killBear && !dataSaved)// if all the sheep are within the radius, then save, send out zeros, and shutdown the node
			{
				shutdownNode();
				dataSaved = true;
				return;
		...
		Eigen::Vector2f xd = Eigen::Vector2f::Zero();
		...
		if (chasing >= 0)
		{
			...
			xd = sheep.at(chasing).getxd();             // goal for the sheep
			
			//std::cout << "hi3\n";
			
			// update the weight estimates
			xBar = x - xd;
			Eigen::Vector2f yd = K1*xBar + xd;
			ey = yd - y;
		...
		if (pushToGoal)
		{
			Eigen::Vector2f Dxg = x - sheep.at(chasing).getxg();
			xNorm = Dxg.norm();
		...
		if ((xNorm <= xNormChasingStart*chasingRatio) || (xNorm < originRadius))
		{
...
int main(int argc, char** argv)
{
	...
	float k1, k2, k3, ks, ky, kcl, height, Gamma, Deltat, chasingRatio, originRadius, velMaxRelDiff, sm, s, lambda;
	int N, L1quad;
	bool pushToGoal;
	...
	nhp.param<float>("chasingRatio",chasingRatio,0.5);
	...
	Bear bear(runNumber,bearName,sheepName,k1,k2,k3,ks,ky,kcl,Gamma,Deltat,N,chasingRatio,height,originRadius,bearWall,pushToGoal,xg,velMaxRelDiff,saveData,sm,s,L1,lambda);
